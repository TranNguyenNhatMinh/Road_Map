(function () {
  const STORAGE_KEY = "frontend-tracker";
  ...
  function getAllModuleIds() {
    if (!window.FRONTEND_TOPICS) return [];
    return window.FRONTEND_TOPICS.flatMap(function (t) {
  ...
  function updateFrontendProgress() {
    var ids = getAllModuleIds();
    var completed = getCompletedSet();
    var total = ids.length;
    var done = completed.size;
    var pct = total ? Math.round((done / total) * 100) : 0;
    var bar = document.getElementById("frontendProgressBar");
    var statDone = document.getElementById("frontendStatDone");
    var statTotal = document.getElementById("frontendStatTotal");
    if (bar) bar.style.width = pct + "%";
    if (statDone) statDone.textContent = done;
    if (statTotal) statTotal.textContent = total;
    var statPct = document.getElementById("frontendStatPct");
  ...
  function showToast(message) {
    var el = document.getElementById("frontendToast");
    if (!el) return;
  ...
  function renderCurriculum() {
    var container = document.getElementById("frontendCurriculum");
    if (!container || !window.FRONTEND_TOPICS) return;
    ...
    progressHtml += "<div class=\"progress-stats\"><span class=\"stat\" id=\"frontendStatDone\">" + done + "</span><span class=\"stat-label\">/</span><span class=\"stat\" id=\"frontendStatTotal\">" + total + "</span><span class=\"stat-label\">items</span><span class=\"stat-pct\" id=\"frontendStatPct\">(" + pct + "%)</span></div></div>";
    progressHtml += "<div class=\"progress-bar-wrap\"><div class=\"progress-bar\" id=\"frontendProgressBar\" style=\"width:" + pct + "%\"></div></div></section>";

    var html = progressHtml;

    window.FRONTEND_TOPICS.forEach(function (topic) {
      html += "<section class=\"topic\" data-topic-id=\"" + topic.id + "\">";
      html += "<h3 class=\"topic-title\">" + escapeHtml(topic.title) + "</h3>";
      ...
      topic.modules.forEach(function (mod) {
        ...
        html += "<div class=\"module-body\"><span class=\"module-title\">" + escapeHtml(mod.title) + "</span></div>";
        html += "</li>";
      });
      html += "</ul></section>";
    });
