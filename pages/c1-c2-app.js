(function () {
  const STORAGE_KEY = "c1-c2-tracker";
  const SECTIONS = window.C1C2_SECTIONS;
  if (!SECTIONS) return;

  const MOTIVATION_BY_PROGRESS = {
    0: "Bắt đầu Unit 1 — từng bước tới C1/C2!",
    ...
    100: "Bạn đã hoàn thành tất cả units. Chúc bạn đạt C1/C2!"
  };

  const COMPLETION_TOASTS = [
    "Unit xong — giữ đà! 💪",
    "Tiến độ tốt! ✨",
    "Cứ thế — C1/C2 đang chờ! 🌟",
  ...
  function showToast(message) {
    const el = document.getElementById("c1c2Toast");
    if (!el) return;
  ...
  function renderCurriculum() {
    ...
    SECTIONS.forEach(function (section) {
      html += "<section class=\"topic c1c2-section\" data-topic-id=\"" + escapeHtml(section.id) + "\">";
      html += "<h3 class=\"topic-title\">" + escapeHtml(section.title) + "</h3>";
